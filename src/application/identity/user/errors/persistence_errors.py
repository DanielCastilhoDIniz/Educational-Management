class ApplicationPersistenceError(Exception):
    """Base exception for persistence-related errors in the user application.
...
class UserPersistenceNotFoundError(ApplicationPersistenceError):
    """
        Raised when an User_id cannot be found
...
class UserDuplicationError(ApplicationPersistenceError):
    """
    Raised when an attempt is made to create an User that already exists,
    violating a uniqueness constraint.
    """


class UserTechnicalPersistenceError(ApplicationPersistenceError):
    """
    Raised for unexpected technical errors during User persistence operations,
