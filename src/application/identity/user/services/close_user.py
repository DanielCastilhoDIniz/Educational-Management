...
class InactivateUserService:
    """Application service to inactivate a user.
    Responsibilities:
    - Orchestrate the process of inactivating a user, including:
    - Retrieving the user aggregate.
    - Emitting appropriate domain events (UserInactivated).
    ...
    def execute(
        ...
        if user is None:
            return build_not_found_result(user_id=user_id, action="inactivate")
        ...
        return finalize_state_change(
            ...
            persistence_failure_message="Failed to persist user inactivating.",
