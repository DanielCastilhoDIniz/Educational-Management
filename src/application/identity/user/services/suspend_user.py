...
class SuspendUserService:
    """Application service to suspend a user.
    Responsibilities:
    - Orchestrate the process of suspending a user, including:
    - Retrieving the user aggregate.
    - Emitting appropriate domain events (UserSuspended).
    ...
    def execute(
        ...
        if user is None:
            return build_not_found_result(user_id=user_id, action="suspend")
        ...
        return finalize_state_change(
            ...
            persistence_failure_message="Failed to persist user suspension.",
