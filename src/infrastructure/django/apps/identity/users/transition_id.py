...
from .constants import IDENTITY_USER_TRANSITION_NS
...
def make_user_transition_id(
    *,
    ...
    """
        Generate a deterministic UUID for an user state transition.
        The UUID is based on the user ID, action, from/to states, timestamp, actor, and justification.
        This ensures that the same transition will always yield the same UUID, which is useful for id
        empotency and traceability.

        Parameters:
        - user_id: The UUID of the user undergoing the transition.
    ...
    return uuid.uuid5(IDENTITY_USER_TRANSITION_NS, fingerprint)
